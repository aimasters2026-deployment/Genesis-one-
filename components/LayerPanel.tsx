...
import { 
  ...
  Group, Ungroup, ChevronDown, Sparkles, Sliders
} from 'lucide-react';
...
const LayerPanel: React.FC<LayerPanelProps> = ({ 
  ...
  if (isMinimized) {
    return (
      <button onClick={() => setIsMinimized(false)} className="absolute right-4 top-16 md:top-20 p-3 rounded-full bg-white/90 dark:bg-slate-800/90 text-slate-600 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-white border border-slate-200 dark:border-slate-700 shadow-xl backdrop-blur-md z-20 transition-all hover:scale-110 group">
         <Layers size={20} />
      </button>
    );
  }
  ...
  return (
    <div className="absolute right-0 md:right-4 top-16 md:top-20 bottom-0 md:bottom-24 w-full md:w-80 bg-white/95 dark:bg-slate-800/90 backdrop-blur-md md:rounded-xl border-l md:border border-slate-200 dark:border-slate-700 shadow-2xl z-30 flex flex-col overflow-hidden animate-in slide-in-from-right duration-300">
      <div className="p-3 border-b border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-900/50 mt-safe md:mt-0">
        <div className="flex items-center justify-between mb-3">
            <h2 className="text-sm font-semibold text-slate-700 dark:text-slate-200 flex items-center gap-2"><Layers size={16} /> Layers</h2>
